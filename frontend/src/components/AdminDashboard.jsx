...
const AdminDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      // FETCH PRODUCTS
      const resProducts = await fetch(`${ADMIN_API}/products`);
      ...
      // FETCH PREBOOKINGS
      const resPre = await fetch(`${ADMIN_API}/prebookings`);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${ADMIN_API}/add-product`, {
        method: "POST",
        body: formData
      });
  ...
  const handleDelete = async (id, type) => {
    ...
    try {
      await fetch(`${ADMIN_API}/delete-${type}/${id}`, { method: "DELETE" });
      loadDashboardData();
    } catch (err) {
